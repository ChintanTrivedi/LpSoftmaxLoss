...
def train_efficientnet(dataset, logdir, num_epochs=50, order='l2', radius='ur', run_exp_count=1):
    ...
    def create_encoder():
        ...
        efficientnet = keras.applications.EfficientNetB0(include_top=False, weights=None, input_shape=input_shape,
                                                         pooling="avg")
        outputs = efficientnet(augmented)
        return keras.Model(inputs=inputs, outputs=outputs, name="efficientnet-encoder")
    ...
    for run_exp in range(run_exp_count):
        ...
        json.dump(train_history.history, open(f"{logdir}/th_ef0_{order}_{radius}_run{run_exp}.json", 'w'), indent=4)
